...
namespace Infrastructure.Services
{
    public class ZoomMeetingRepository : IZoomMeetingRepository
    {
        ...
        public ZoomMeetingRepository(ApplicationDbContext context, ILogger<ZoomMeetingRepository> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        public async Task<ZoomMeeting> CreateMeetingAsync(ZoomMeeting meeting, CancellationToken ct = default)
        {
            try
            {
                ...
                _logger.LogInformation("Successfully created Zoom meeting with ID: {MeetingId}", meeting.ZoomMeetingId);
                return meeting;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> UpdateMeetingAsync(ZoomMeeting meeting, CancellationToken ct = default)
        {
            try
            {
                ...
                _logger.LogInformation("Successfully updated Zoom meeting with ID: {MeetingId}", meeting.ZoomMeetingId);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> DeleteMeetingAsync(long meetingId, CancellationToken ct = default)
        {
            try
            {
                ...
                _logger.LogInformation("Successfully deleted Zoom meeting with ID: {MeetingId}", meetingId);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<ZoomRecording> SaveRecordingAsync(ZoomRecording recording, CancellationToken ct = default)
        {
            try
            {
                ...
                await _context.SaveChangesAsync(ct);
                _logger.LogInformation("Successfully saved Zoom recording with ID: {RecordingId}", recording.RecordingId);
